...
export const getPrediction = async (req, res) => {
    try {
        ...
        const { team1, team2, team1Spells, team2Spells } = req.body;
...
export const getAllChampions = async (req, res) => {
    try {
        ...
        const championsArray = Object.values(data).filter((c) => c.id !== -1);
...
export const getChampionById = async (req, res) => {
    try {
        ...
        const json = JSON.parse(rawData);
        const data = json.data || json;
        const champion = Object.values(data).find((c) => c.id == id);
        if (!champion) {
