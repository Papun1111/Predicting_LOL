    ...
    <main className="min-h-screen text-white pb-20 selection:bg-[#00f2ff]/30 relative overflow-hidden">
      <div className="relative z-10 w-full">
        ...
        <div className="container mx-auto px-4 py-10 max-w-7xl">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-end gap-6 mb-12">
            ...
            >
              <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter mb-2 relative">
                <span className="relative z-10 drop-shadow-[0_0_30px_rgba(0,242,255,0.3)] text-white">CHAMPION</span>
                <br className="md:hidden" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00f2ff] to-[#a855f7] ml-0 md:ml-4">
                  ROSTER
                </span>
              </h1>
              <p className="text-slate-400 mt-2 max-w-xl font-light text-sm md:text-base">
                Explore the <span className="text-[#00f2ff] font-bold">legends of Runeterra</span>. Search, filter, and analyze the heroes available for your draft.
              </p>
            </motion.div>
            ...
            >
              <div className="relative group w-full lg:w-[400px]">
                <Search className="absolute left-4 top-3.5 text-slate-500 group-focus-within:text-[#00f2ff] transition-colors" size={20} />
                <input 
                  type="text" 
                  placeholder="Initialize search sequence..." 
                  className="bg-black/60 backdrop-blur-md border border-white/10 rounded-xl pl-12 pr-4 py-3 w-full focus:outline-none focus:border-[#00f2ff]/50 focus:shadow-[0_0_20px_rgba(0,242,255,0.2)] transition-all text-sm font-mono text-white"
                  value={search}
                ...
                {roles.map(role => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-lg text-xs font-bold uppercase tracking-[0.1em] transition-all duration-300 border flex-shrink-0 ${
                      roleFilter === role 
                      ? 'bg-gradient-to-r from-[#00f2ff]/20 to-[#a855f7]/20 border-[#00f2ff]/50 text-white shadow-[0_0_15px_rgba(0,242,255,0.3)] scale-105' 
                      : 'bg-black/40 backdrop-blur-sm border-white/5 text-slate-400 hover:bg-white/10 hover:text-white hover:border-[#00f2ff]/30'
                    }`}
                  >
                    {role}
                  </button>
          ...
          {/* Grid Section */}
          {loading ? (
             <div className="flex flex-col items-center justify-center py-32 text-[#00f2ff]">
               <motion.div 
                 animate={{ rotate: 360 }}
                  ...
                  >
                    <Link href={`/dashboard/${champ.id}`} className="block h-full">
                      <motion.div 
                        ...
                        whileTap={{ scale: 0.98 }}
                        className="group relative h-full rounded-2xl overflow-hidden border border-white/10 bg-black/60 backdrop-blur-xl transition-all duration-300 hover:border-[#00f2ff]/50 hover:shadow-[0_0_30px_rgba(0,242,255,0.2)] flex flex-col"
                      >
                        ...
                        <div className="h-64 w-full relative overflow-hidden bg-slate-900 border-b border-white/5">
                          ...
                          {/* Inner Hover Glow */}
                          <div className="absolute inset-0 bg-[#00f2ff] mix-blend-overlay opacity-0 group-hover:opacity-20 transition-opacity duration-500" />
                        </div>
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-5 transform translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
                          <h3 className="text-2xl font-black text-white group-hover:text-[#00f2ff] transition-colors uppercase tracking-widest leading-none drop-shadow-md">
                            {champ.name}
                          </h3>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {champ.tags && champ.tags.map((tag) => (
                              <span key={tag} className="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded bg-black/60 border border-white/20 text-slate-300 group-hover:border-[#00f2ff]/30 transition-colors">
                                {tag}
                              </span>
                            ))}
                          </div>
